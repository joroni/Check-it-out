...
/*	$( '<div class="clearfix"><div class="clearfix pull-right" id="user-info"></div></div>' ).insertAfter( '#feedback' );
	...
	$( '<div class="clearfix" id="user-info"></div>' ).insertBefore('#app-menu');
	var $user_info = $('#user-info');
	...
	var update_login_info = function() {
		...
		if ( current_user ) { 
			...
			$user_info.html( '<span style="float:left; margin-right: 10px;">Logged in as <a href="#user-page" style="color:red;">'+ current_user.login +'</a></span><button type="button" class="btn btn-danger" id="logout" style="display: block; width: auto !important; float: left;">Log out</button>');
		} else { 
			...
			$user_info.html( '<ul class="nav navbar-nav"><li><a href="#" id="login">Log in</a></li></ul>' );
		}
	...
	$( $user_info ).on( 'click', '#login', function( e ) {
		e.preventDefault();
		$user_info.html( 
			'<input id="userlogin" placeholder="User ID" type="text" >' +
			'<input id="userpass" placeholder="Password" type="password" >' +
			'<button type="button" class="btn btn-danger" id="go-login" style="width:auto !important;">Log in</button>'
		);
	} );
